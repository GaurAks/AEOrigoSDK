...
SWIFT_CLASS("_TtC8OrigoSDK9GeoRegion")
@interface GeoRegion : NSObject <NSCoding>
...
- (void)requestLocationPermissionsIfRequired;
@end
...
+ (void)redeem:(IssuanceTokenParameters * _Nonnull)parameters supportedOIPType:(NSArray<NSString *> * _Nonnull)supportedOIPType completion:(void (^ _Nonnull)(NSDictionary<NSString *, NSString *> * _Nullable, NSError * _Nullable))completion;
...
+ (void)getPassStatus:(NSString * _Nonnull)appId completion:(void (^ _Nonnull)(BOOL, NSError * _Nullable))completion;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// 10704, The server communication failed due to access denied
  OrigoKeysNetworkErrorCodeErrorCodePassRevoked = 10704,
...
@interface OrigoLocationDataHandler (SWIFT_EXTENSION(OrigoSDK))
- (void)addReader:(NSArray<NSString *> * _Nullable)psnList;
@end
...
+ (NSURL * _Nonnull)getReportURL SWIFT_WARN_UNUSED_RESULT;
+ (void)walletStart;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// Mark:- this method once optimus implemented kAttrAccessible in keychain
+ (void)clearKeyChain;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
SWIFT_EXTERN void debugger_callback(void);


SWIFT_EXTERN void jailbreak_callback(void);
